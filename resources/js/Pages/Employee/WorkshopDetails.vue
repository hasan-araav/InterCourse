...
const register = () => {
    router.post(route('workshops.register', props.workshop.id));
};

const cancel = () => {
    if (confirm('Are you sure you want to cancel your registration?')) {
        router.delete(route('workshops.cancel', props.workshop.id));
    }
};
</script>
    ...
    <EmployeeLayout>
        <template #header>
            <div class="flex items-center justify-between py-4">
                ...
                <div v-if="workshop.user_registration" class="flex items-center space-x-2">
                    <span
                        :class="[
                            workshop.user_registration.status === 'confirmed' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-amber-50 text-amber-700 border-amber-100',
                            'px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full border shadow-sm'
                        ]"
                    >
                        {{ workshop.user_registration.status === 'confirmed' ? 'Confirmed' : 'Waitlisted #' + workshop.user_registration.position }}
                    </span>
                </div>
            </div>
        </template>
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white rounded-[3rem] overflow-hidden shadow-sm border border-gray-100">
                <div class="relative h-80 lg:h-96">
                    <img :src="workshop.cover_photo_url" class="w-full h-full object-cover" />
                    ...
                    <div class="absolute bottom-0 left-0 p-8 lg:p-12 w-full">
                        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
                            <div class="space-y-4">
                                ...
                                <h1 class="text-4xl lg:text-6xl font-black text-white tracking-tight leading-tight">{{ workshop.title }}</h1>
                            ...
                            <div class="flex-shrink-0">
                                ...
                                <div v-else>
                                    <button
                                        @click="register"
                                        :class="workshop.remaining_seats > 0 ? 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200' : 'bg-amber-500 hover:bg-amber-600 shadow-amber-200'"
                                    ...
                                    >
                                        {{ workshop.remaining_seats > 0 ? 'Register Now' : 'Join Waitlist' }}
                ...
                <div class="p-8 lg:p-16">
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-16">
                        ...
                        <div class="lg:col-span-2 space-y-16">
                            ...
                            <div v-if="workshop.speaker_name" class="space-y-8 pt-8 border-t border-gray-100">
                                <div class="flex items-center space-x-3 text-indigo-600">
                                    <Users class="h-6 w-6" />
                                ...
                                <div class="bg-indigo-50/50 rounded-[3rem] p-10 flex flex-col md:flex-row items-center md:items-start gap-10 border border-indigo-100">
                                    <div class="relative">
                                        <img :src="workshop.speaker_photo_url" class="h-32 w-32 rounded-[2.5rem] object-cover shadow-xl ring-4 ring-white" />
                                    ...
                                    <div class="space-y-4 text-center md:text-left flex-1">
                                        <h3 class="text-3xl font-black text-gray-900 tracking-tight">{{ workshop.speaker_name }}</h3>
                                        <p class="text-gray-600 leading-relaxed text-lg font-medium">{{ workshop.speaker_bio || 'Expert instructor at InterCourse Academy.' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="bg-white rounded-[2.5rem] p-10 space-y-8 h-fit sticky top-12 shadow-sm border border-gray-100">
                                ...
                                <div class="space-y-4">
                                    <div class="p-6 bg-gray-50 rounded-3xl border border-gray-100 transition-all hover:shadow-md">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span class="text-2xl font-black text-gray-900">{{ workshop.capacity }}</span>
                                        </div>
                                    </div>
                                    <div class="p-6 bg-gray-50 rounded-3xl border border-gray-100 transition-all hover:shadow-md">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span class="text-2xl font-black text-emerald-600">{{ workshop.confirmed_count }}</span>
                                        </div>
                                    </div>
                                    <div class="p-6 bg-gray-50 rounded-3xl border border-gray-100 transition-all hover:shadow-md">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <span class="text-2xl font-black text-amber-600">{{ workshop.capacity - workshop.remaining_seats - workshop.confirmed_count > 0 ? (workshop.capacity - workshop.remaining_seats - workshop.confirmed_count) : 0 }}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="pt-4">
                                    <div class="flex justify-between text-xs font-black uppercase tracking-widest text-gray-400 mb-3">
                                        ...
                                        <span>{{ Math.round((workshop.confirmed_count / workshop.capacity) * 100) }}%</span>
                                    </div>
                                    <div class="h-3 w-full bg-gray-200 rounded-full overflow-hidden">
                                        <div
                                            ...
                                            :style="{ width: Math.round((workshop.confirmed_count / workshop.capacity) * 100) + '%' }"
                                        ></div>
                                    </div>
                                </div>
                            </div>
